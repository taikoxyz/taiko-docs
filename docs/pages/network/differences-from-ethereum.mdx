...
Taiko runs the **Osaka** EVM. The Cancun, Prague, and Osaka execution-layer features are **available**:

| Feature                                  | Status    |
| ---------------------------------------- | --------- |
| `TSTORE` / `TLOAD` (transient storage)   | Available |
| `MCOPY`                                  | Available |
| `BLOBHASH`                               | Available |
| `BLOBBASEFEE`                            | Available |
| BLS12-381 precompiles (`0x0b`–`0x11`)    | Available |
| `CLZ` (`0x1e`)                           | Available |
| `P256VERIFY` (`0x100`)                   | Available |

Solidity 0.8.24+ defaults to Cancun, which runs on Taiko. To use Prague and Osaka features and keep builds deterministic, set the EVM version to `osaka` explicitly.
...
OpenZeppelin Contracts v5 uses transient storage (`TSTORE`/`TLOAD`) in the `ReentrancyGuard` and other contracts. These work on Taiko's Osaka EVM without modification.
...
- No L1 blob fees are charged for L2 transactions.
