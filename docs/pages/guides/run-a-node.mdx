...
Taiko nodes still follow Ethereum's execution/consensus split, but under the Shasta protocol architecture (which the current Unzen fork runs on) the consensus side is driven by Taiko's L1 inbox, proposal derivation rules, and L2 anchoring logic instead of an Ethereum beacon chain. The main components are:
...
- [`taiko-client`](https://github.com/taikoxyz/taiko-mono/tree/main/packages/taiko-client) — the **consensus and rollup driver**. It watches the `Inbox` on L1, derives L2 blocks from proposal data, tracks finalization, and drives the selected execution client through the Engine API.

Since the Shasta fork, the protocol no longer upgrades the old Pacaya inbox in place. It uses a separate **Inbox** contract on L1 for proposing, proving, forced inclusion, and finalization, while the L2 side uses the **TaikoAnchor** plus fork-router contracts to keep pre-fork and post-fork behavior compatible during the transition.
...
| `proposer` | Collects transactions and submits proposals to the L1 inbox, including lookahead data when preconfirmations are enabled |
...
6. Keep the L2 head anchored to the L1 checkpoints exposed through the anchor path
