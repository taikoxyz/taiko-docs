...
Taiko nodes follow Ethereum's execution/consensus separation model, but replace the beacon-chain consensus with Taiko-specific rollup logic. The two main components are:

- [`taiko-geth`](https://github.com/taikoxyz/taiko-geth) — a minimally modified fork of `go-ethereum` that serves as the **execution client**. It executes L2 transactions, maintains chain state, and exposes standard Ethereum JSON-RPC interfaces.
- [`taiko-client`](https://github.com/taikoxyz/taiko-mono/tree/main/packages/taiko-client) — the **consensus and rollup driver** that replaces the beacon chain. It watches L1, reconstructs L2 batches, and drives `taiko-geth` through the Engine API.
...
./bin/taiko-client driver \
  ...
  --preconfirmation.whitelist 0x8B969Fcf37122bC5eCB4E0e5Ad65CEEC3f1393ba \
