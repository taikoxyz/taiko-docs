...
- [`taiko-geth`](https://github.com/taikoxyz/taiko-geth) — a minimally modified fork of `go-ethereum` that serves as the **execution client**. It executes L2 blocks, maintains chain state, and exposes standard Ethereum JSON-RPC interfaces.
- [`taiko-client`](https://github.com/taikoxyz/taiko-mono/tree/main/packages/taiko-client) — the **consensus and rollup driver**. It watches the Shasta inbox on L1, derives L2 blocks from proposal data, tracks finalization, and drives `taiko-geth` through the Engine API.
