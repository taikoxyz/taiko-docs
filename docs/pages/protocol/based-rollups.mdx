...
- **Proposal inclusion still inherits Ethereum ordering.** In the current Unzen rollout, the normal proposing path is preconf-whitelisted, but the proposal transactions themselves are still ordered by Ethereum L1, and forced inclusion preserves a permissionless fallback path.
...
Under the **Shasta** protocol architecture (which the current Unzen fork runs on), proposers interact with the **Inbox** contract deployed on Ethereum L1. The process works as follows:
...
5. In the current Unzen rollout, normal proposing is gated by the preconfirmation whitelist. If forced inclusions remain old enough, proposing becomes permissionless as a fallback.
...
:::info
Under the Pacaya fork, blocks were proposed in **batches** rather than individually. The Shasta fork switched to **proposal-based derivation** around the separate `Inbox`, while keeping Ethereum L1 as the ordering layer; Unzen keeps that model.
:::
...
Even during the current preconfirmation phase (where whitelisted preconfers sequence blocks), Taiko maintains censorship resistance through **forced inclusion**, built into the `Inbox` contract on L1. Any user can call `saveForcedInclusion` with a blob reference to queue transactions; once a forced inclusion is due, the next proposal must process it before the proposer's own transactions, regardless of whether the current preconfer chooses to include it.
