...
| **ZkRequiredVerifier** (ComposeVerifier) | Verifier contract that orchestrates multiple sub-verifiers (SGX, ZK) for multi-proof validation and requires at least one ZK proof per proposal range. |
...
Provers generate validity proofs for a contiguous range of proposals. The proof submitted to `Inbox.prove` is checked through a composed verifier that requires two sub-proofs with at least one ZK proof, such as `sgxGeth + RISC0`, `sgxReth + SP1`, or `RISC0 + SP1`.
...
- **ZkRequiredVerifier** (a ComposeVerifier) orchestrates multi-proof verification and requires at least one ZK proof per range
- **Forced inclusion** (built into the Inbox) preserves censorship resistance during the preconfirmation rollout
