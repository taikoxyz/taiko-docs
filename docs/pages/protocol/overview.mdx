...
Under Shasta, the lifecycle of a Taiko block is best understood as **proposed, proved, and finalized**.
...
Provers generate validity proofs for a contiguous range of proposals. Under the current multi-proof system, the proof submitted to `Inbox.prove` is checked through a composed verifier, with sufficient proof pairs such as `sgxGeth + sgxReth`, `sgxGeth + RISC0`, or `sgxReth + SP1`.

The `Proved` event marks the range that was successfully covered by the proof submission.

### 3. Finalized

In Shasta, successful proving advances finalization directly. `Inbox` checks that the proven range links to the current finalized head, writes a checkpoint into the signal service, and updates the finalized proposal ID and block hash in order.

A finalized proposal range has crossed the protocol's main correctness checkpoint on L1.
...
| **Proved** | A valid proof submission has been accepted for a proposal range. | Stronger -- correctness has been checked by the configured verifier set. |
| **Finalized** | The proven range has advanced the finalized proposal head and written a checkpoint to the signal service. | Final at the protocol level, with L1-backed checkpointing. |
...
- [Pacaya Fork](/protocol/pacaya-fork) -- the earlier batch-based protocol design
