...
Unzen runs the **Osaka** EVM version, which makes the Cancun, Prague, and Osaka execution-layer features available to contracts.

The main contract-facing additions are:

- Cancun opcodes: `TSTORE`, `TLOAD`, and `MCOPY` (`BLOBHASH` and `BLOBBASEFEE` also execute but always return `0` and `1`: Taiko L2 rejects blob transactions and never carries blobs)
...
## Proof Verifier: ZK Required

With Unzen (Proposal0019), the mainnet `Inbox` points at `ZkRequiredVerifier`, which accepts two sub-proofs only if at least one of them is a ZK proof (RISC0 or SP1). A TEE-only pair no longer finalizes a range. See [Proving System](/protocol/proving-system).
