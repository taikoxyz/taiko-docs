# Proving System [Taiko's proving lifecycle — Inbox proposals, SGX and ZK multi-proof verification, and checkpoint-driven L1 finalization for L2 state.]

Under the **Shasta** protocol architecture (which the current Unzen fork runs on), Taiko proves L2 state transitions on Ethereum L1 through the `Inbox` and a compose `proofVerifier`. The core idea is still proof diversity, but the lifecycle is no longer the old Pacaya-style `proveBatches` plus `verifyBatches` flow. A successful proof submission advances finalization directly for a contiguous range of proposals.
...
The inbox does not hardcode a single proving system. Instead, it points at a verifier contract that implements `IProofVerifier`. The current Unzen deployment on mainnet uses `ZkRequiredVerifier`, a `ComposeVerifier` over multiple sub-verifiers that requires at least one ZK proof in every accepted combination.

The deployed verifier set includes:
...
`ZkRequiredVerifier` accepts exactly two sub-proofs, and the second must be a ZK proof:
...
A TEE-only pair (`sgxGeth + sgxReth`) is not accepted. This keeps proof diversity while guaranteeing that every finalized range carries a cryptographic proof.
...
- **Client diversity** matters too: the verifier set spans both geth-based and reth-based proving paths.
...
On the live mainnet Unzen inbox, the proving-related timing parameters are:
...
For the current live Unzen configuration, however, this path is effectively dormant because `livenessBond = 0` and proving runs with whitelist mode enabled.
...
Proposer submits proposal(s) to Inbox
 ...
 Inbox validates range + prover permissions
        |
        v
 ZkRequiredVerifier (ComposeVerifier) routes proof pieces
...
Taiko's proving system is defined by:

- proposal-range proving through `Inbox.prove`
- direct finalization on successful proof submission
- a compose verifier that requires two sub-proofs, at least one of them ZK
- whitelist-first prover access with permissionless fallback after delay

For the broader protocol context, see [Shasta Fork](/protocol/shasta-fork), [Unzen Fork](/protocol/unzen-fork), [Based Rollups](/protocol/based-rollups), and [Contract Addresses](/network/contract-addresses).
