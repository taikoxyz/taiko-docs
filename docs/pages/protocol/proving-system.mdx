# Proving System [Taiko's Unzen proving lifecycle — Inbox proposals, SGX and ZK multi-proof verification, and checkpoint-driven L1 finalization for L2 state.]

Under the **Unzen** architecture, Taiko proves L2 state transitions on Ethereum L1 through the `Inbox` and a compose `proofVerifier`. The core idea is still proof diversity, but the lifecycle is no longer the old Pacaya-style `proveBatches` plus `verifyBatches` flow. In Unzen, a successful proof submission advances finalization directly for a contiguous range of proposals.

## Unzen Proving Model

Unzen organizes L2 data around **proposals** rather than the older batch terminology.
...
This means there is no separate public "verified later by cooldown" step in the Unzen inbox flow. Successful proving is what advances finalization.

## Proof Verifier Composition

The inbox does not hardcode a single proving system. Instead, it points at a verifier contract that implements `IProofVerifier`. The current Unzen deployment uses a `ComposeVerifier` over multiple sub-verifiers.
...
- **Client diversity** matters too: the Unzen verifier set spans both geth-based and reth-based proving paths.

The protocol therefore does not rely on a single verifier family or a single execution client to advance finalization.

## Prover Permissions

Unzen is designed for the staged preconfirmation rollout, so proving is not simply "anyone at any time".
...
Under Unzen, Taiko's proving system is defined by:
