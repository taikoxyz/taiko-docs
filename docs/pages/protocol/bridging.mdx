...
:::warning
**L2 to L1 withdrawals wait for proof submission.** Shasta's proving-window target is 4 hours, so expect withdrawals to take at least several hours on the normal path. ERC20Vault supports solver-based fast withdrawals to mitigate this delay for ERC-20 tokens.
:::
