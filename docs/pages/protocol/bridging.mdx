...
The bridge implements **rate limits** on withdrawals via the **QuotaManager** contract. This prevents excessive outflows that could indicate an exploit.

Example quotas:

| Asset | Quota | Period |
|-------|-------|--------|
| **ETH** | 1,000 ETH | 24 hours |
| **USDC** | 4,000,000 USDC | 24 hours |

If a withdrawal would exceed the quota, the message processing returns an `OUT_OF_ETH_QUOTA` status reason. The message enters the RETRIABLE state and can be retried after the quota replenishes.
